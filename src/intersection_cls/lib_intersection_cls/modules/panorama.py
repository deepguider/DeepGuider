...
class PanoramicImageTool(object):
    ...
    def set_panoramic_image_texture_using_index_maps(self, color_map, cube_coords, face_index_map, shape):
        ...
        print('panoramic image shape: ', panoramic_image.shape)
        print('color map shape: ', color_map.shape)
        print('color map ravel shape: ', color_map[0, ...].ravel(order='F').shape)
        print('sub2ind shape: ', sub2ind.shape)

        for i in range(3):
            print('panoramic image [..., i] shape: ', panoramic_image[..., i].shape)
            print('color map ravel sub2ind shape: ', color_map[i, ...].ravel(order='F')[sub2ind].shape)
...
if __name__ == '__main__':
    ...
    cv2.imwrite('/home/dg/Downloads/220628_COEX_Indoor_workshop/rosbag/_2022-06-28-17-03-29_pathA1_back/bottom_000000.jpg', crop_image)
