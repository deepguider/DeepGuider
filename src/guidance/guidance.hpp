...
namespace dg
{
		...
		struct ExtendedPathElement
		{
			...
			*/
			ExtendedPathElement(ID _fromnode, ID _curedge, ID _tonode,
				...
				next_edge_id(_nextedge), cur_degree(_degree) {}
	...
	public:
		...
		bool initiateNewGuidance(Path& path, Map& map);
	...
	protected:
		bool validatePath(Path& path, Map& map);
		int getDegree(Node* node1, Node* node2, Node* node3);
