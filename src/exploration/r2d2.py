...
def uniform_quat(original_angle):
    ...
    euler_angles = np.array([original_euler[0],
                            (np.random.rand() - 0.5) * 2*np.pi / 9. + original_euler[1],
                             original_euler[2]])
...
class NonMaxSuppression (torch.nn.Module): # batchwise
    ...
    def forward(self, reliability, repeatability, **kw):
        ...
        return maxima.nonzero().t()

class NonMaxSuppressionSingle (torch.nn.Module):
    def __init__(self, rel_thr=0.7, rep_thr=0.7):
        nn.Module.__init__(self)
        self.max_filter = torch.nn.MaxPool2d(kernel_size=3, stride=1, padding=1)
        self.rel_thr = rel_thr
...
def extract_multiscale( net, img, detector, scale_f=2**0.25, # batchwise extraction: basenet in patchnet.py must be modified
                        min_scale=0.0, max_scale=1, 
                        min_size=256, max_size=1024, 
                        verbose=False, top_k=16):
    ...
    # XX,YY,SS,CC,QQ,DD = [],[],[],[],[],[] # batchwise
    B = int(B)
    X,Y,S,C,Q,D = [[] for i in range(B)], [[] for i in range(B)], [[] for i in range(B)], [[] for i in range(B)], [[] for i in range(B)], [[] for i in range(B)]
    while  s+0.001 >= max(min_scale, min_size / max(H,W)):
        if s-0.001 <= min(max_scale, max_size / max(H,W)):
            ...
            b,ch,y,x = detector(**res) # nms
            assert ch.cpu().numpy().all() == 0, "should have single channel" ### batch wise save?
            c = reliability[b,ch,y,x]
            q = repeatability[b,ch,y,x]
            # print(b.shape, ch.shape, y.shape, x.shape)
            # print(descriptors[b,:,y,x].shape)
            d = descriptors[b,:,y,x] #t()
            n = d.shape[0] #[0] # shape d: (batch, number of keypoints, desc dimension)

            ## TODO: we should make batchwise lists of descriptors, keypoints, scores. 
            for batch in range(B):
                # accumulate multiple scales  
                X[batch].append(x[b==batch].float() * W/nw)
                Y[batch].append(y[b==batch].float() * H/nh)
                S[batch].append((32/s) * torch.ones(n, dtype=torch.float32, device=d.device))
                C[batch].append(c[b==batch])
                Q[batch].append(q[b==batch])
                D[batch].append(d[b==batch])
        s /= scale_f
    ...
    XYS, scores = [], []
    for batch in range(B):      
        score = torch.cat(C[batch]) * torch.cat(Q[batch])  # scores = reliability * repeatability
        idxs = score.argsort()[-top_k or None:]
        scores.append(score[idxs])
        Y[batch] = torch.cat(Y[batch])[idxs]
        X[batch] = torch.cat(X[batch])[idxs]
        S[batch] = torch.cat(S[batch])[idxs] # scale
        XYS.append( torch.stack([X[batch], Y[batch]], dim=-1) )
        D[batch] = torch.cat(D[batch])[idxs]#.t()
    return {'keypoints': XYS, 'descriptors': D, 'scores': scores}

def extract_multiscale_single( net, img, detector, scale_f=2**0.25, 
                        min_scale=0.0, max_scale=1, 
                        min_size=256, max_size=1024, 
                        verbose=False):
    old_bm = torch.backends.cudnn.benchmark 
    ...
    while  s+0.001 >= max(min_scale, min_size / max(H,W)):
        if s-0.001 <= min(max_scale, max_size / max(H,W)):
            ...
            repeatability = res['repeatability']
