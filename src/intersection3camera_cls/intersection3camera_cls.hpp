#ifndef __INTERSECTION3CAMERA_CLS__
#define __INTERSECTION3CAMERA_CLS__
...
namespace dg
{
    struct Intersection3CameraResult
    {
    ...
    */
    class Intersection3CameraClassifier: public PythonModuleWrapper
    {
    public:
        bool initialize(const char* py_module_path = "./../src/intersection3camera_cls", const char* module_name = "intersection3camera_cls", const char* class_name = "Intersection3CameraClassifier")
        {
        ...
        */
        bool _apply(cv::Mat image, dg::Timestamp ts)
        {
            ...
            if (pRet != NULL) {
                ...
                if (n_ret != 2)
                {
                    fprintf(stderr, "Intersection3CameraClassifier::apply() - Wrong number of returns\n");
                    return false;
                }

                cv::AutoLock lock(m_mutex);
                // intersection3camera class & confidence
                ...
                PyErr_Print();
                fprintf(stderr, "Intersection3CameraClassifier::apply() - Call failed\n");
                return false;
            }
        ...
        Intersection3CameraResult get() const
        {
            cv::AutoLock lock(m_mutex);
            return m_result;
        }

        Intersection3CameraResult get(Timestamp& ts) const
        {
            cv::AutoLock lock(m_mutex);
            ts = m_timestamp;
            return m_result;
        ...
        void set(const Intersection3CameraResult& intersect, Timestamp ts, double proc_time = -1)
        {
            cv::AutoLock lock(m_mutex);
            m_result = intersect;
            m_timestamp = ts;
        ...
        void print() const
        {
            ...
            printf("\tintersection3camera: %d (%.2lf)\n", m_result.cls, m_result.confidence);
        }
        ...
        void read(const std::vector<std::string>& stream)
        {
            ...
            for (int k = 0; k < (int)stream.size(); k++)
            {
                ...
                if (elems.size() != 6)
                {
                    printf("[intersection3Camera] Invalid log data %s\n", stream[k].c_str());
                    return;
                }
        ...
        {
            return "intersection3camera";
        }


    protected:
        Intersection3CameraResult m_result;
    };

} // End of 'dg'

#endif // End of '__INTERSECTION3CAMERA_CLS__'
