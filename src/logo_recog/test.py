...
def main(args):
    ...
    if args.mode.lower() == 'detect':
        ...
        for cat in tqdm(cat_list):
            ...
            cat_cnt = 0
            if not os.path.exists(os.path.join(roi_path, base)):
                os.mkdir(os.path.join(roi_path, base))
                
            for img_path in img_list:
                pred, _, t = detect_logo_only(str(img_path), yolo, 
                                 save_img=False, save_img_path=args.result_path)
                ...
                if img is not None:
                    for p in pred:
                        ...
                        output_path = os.path.join(roi_path, base, base + str(cat_cnt) + '.jpg')
