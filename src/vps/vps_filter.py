...
class vps_filter:
    def __init__(self):
        ...
        self.n_samples = 20  # filter size of points
        self.n_samples_max = 100  # maximum filter size of points
        self.dims = 2
        ...
        if True:
            self.ransacline_distance_threshold = 10 # noisy postion difference in meters
            self.utm_distance_threshold = 7
        else:  # Large number means disabling this filter
            self.ransacline_distance_threshold = 100 # noisy postion difference in meters
            self.utm_distance_threshold = 100
    ...
    def check_valid(self, new_x, new_y):
        ...
        utm_xys = self.get_samples()
        if len(utm_xys) < self.n_samples:
            return False
        try:
            ransac_model, inliers = ransac(utm_xys, LineModelND, min_samples=2, residual_threshold=1, max_trials=1000)
            #ransac.fit(utm_xys[:,0].reshape(-1,1), utm_xys[:,1].reshape(-1,1))
            ...
            if d <= self.ransacline_distance_threshold:  # Check new point is near to ransac line
                mean_xys = np.median(utm_xys[inliers], axis=0)
