...
int DeepGuider::run()
{
    ...
    std::string ahrs_file, ocr_file, poi_file, vps_file, intersection_file, intersection3camera_file, roadlr_file, roadtheta_file, exploration_file;
    ...
    if (!data_loader.load(m_video_input_path, m_gps_input_path, ahrs_file, ocr_file, poi_file, vps_file, intersection_file, intersection3camera_file, roadlr_file, roadtheta_file))
    {
...
bool DeepGuider::get_360cam_image(cv::Mat& cam_image, cv::Mat& cam_image_for_draw, double& txt_scale, dg::Timestamp& capture_time, dg::Timestamp capture_time_prev)
{
	int num_of_hstack_image = 3;
   	m_cam_mutex.lock();
	capture_time = m_cam_capture_time;
	...
	if (m_enable_360cam)  // use 360cam (uncropped)
	{
    	m_360cam_mutex.lock();
	    if(m_360cam_image.empty() || capture_time <= capture_time_prev)
	    {
	        m_360cam_mutex.unlock();
	        return false;
	    }
