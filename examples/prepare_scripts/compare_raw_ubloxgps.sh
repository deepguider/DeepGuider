...
run_parse_ubloxgps "$rosbag_file" "$output_dir"
...
echo " >>> Green color points are for 2nd detected [GPS] topic"
if [ -e ${extracted_dir}/poses_latlon_robot_ubloxgps_raw.txt ];then
	echo "color green" >> ${extracted_dir}/${poses_latlon_compare_file}
	cat ${extracted_dir}/poses_latlon_robot_ubloxgps_raw.txt >> ${extracted_dir}/${poses_latlon_compare_file}
fi
