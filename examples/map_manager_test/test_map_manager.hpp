...
int testSimpleMapManager()
{
	...
	ok = manager.getPOI(559542564800095, 500.0, poi_list);
	if (ok)
		VVS_CHECK_EQUL(poi_list.size(), 212);
	dg::POI poi;
	...
	ok = manager.getStreetView(559542564800095, 500.0, sv_list);
	if (ok)
		VVS_CHECK_EQUL(sv_list.size(), 662);
	dg::StreetView sv;
