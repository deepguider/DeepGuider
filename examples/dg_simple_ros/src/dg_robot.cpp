...
protected:
    ...
    bool makeSubgoal7(Pose2& pub_pose);
    bool makeSubgoal8(Pose2& pub_pose);
    bool makeSubgoal9(Pose2& pub_pose);
    bool makeSubgoal10(Pose2& pub_pose);
    bool makeSubgoal11(Pose2& pub_pose);
...
void DGRobot::publishSubGoal3()
{
    ...
    {         
        ...
        if (duration > ros::Duration(5.0))
        {
            ...
            // if (makeSubgoal(pub_pose))  // Robot's coordinate  
            if (makeSubgoal11(pub_pose))  // Robot's coordinate  
            {
                geometry_msgs::PoseStamped rosps = makeRosPubPoseMsg(m_cur_head_node_id, pub_pose);
                pub_subgoal.publish(rosps);
                m_guider.m_subgoal_pose = pub_pose;
...
bool DGRobot::makeSubgoal(Pose2& pub_pose)
{
    ...
    {
        ROS_INFO("Current node is in dark area: %d", value);
    ...
    cv::drawMarker(colormap, next_node_px, cv::Vec3b(0, 100, 255), cv::MARKER_DIAMOND);    //cur node: orange diamond
    cv::line(colormap, robot_px, next_node_px, cv::Vec3b(0, 100, 255), 2);  

    cv::drawMarker(colormap, cur_node_px, cv::Vec3b(0, 255, 255), cv::MARKER_DIAMOND); //next node: yellow diamond
    cv::line(colormap, cur_node_px, dg_px, cv::Vec3b(0, 255, 255), 2);  
      
    Point2 new_node_px = cvtMetric2Pixel(new_node_dx);
    cv::drawMarker(colormap, new_node_px, cv::Vec3b(0, 0, 255), cv::MARKER_DIAMOND); //new node: orange diamond
    cv::line(colormap, robot_px, new_node_px, cv::Vec3b(0, 0, 255), 2);

    if (norm(node_px_moved - Point2(0,0))>1)
    {
        cv::drawMarker(colormap, node_px_moved, cv::Vec3b(255, 0, 255), cv::MARKER_DIAMOND); //new moved node: magenta 
    }    
    ...
    if (x+m_framesize_crop.width >= colormap.cols) x = colormap.cols - m_framesize_crop.width - 1;
    if (x<=1) x = 1;
    if (y+m_framesize_crop.height >= colormap.rows) y = colormap.rows - m_framesize_crop.height - 1;
    if (y<=1) y = 1;
...
{ 
    ...
    if (m_cur_head_node_id != next_guide.cur_node_id)  // new dg_next_node_robot
    {
        ...
        m_cur_robot_pose = robot_pose;
        m_robotarrived_but_nodenotyetupdated = false; // node updated 
        m_guider.m_robot_heading_node_pose = next_node_robot;
    }
    else{  // if not new node
...
{ 
    ...
    ////////////////////////////////////////////////////////////////////////////////////////////////////
    // // Plan A-2: comment below
    // Plan B-2-1: uncomment below (still have problem: what if the pub pose is not in drivable area? Try to go to next next node?)
    if (dist_robot_to_nextnode < 1 || m_robotarrived_but_nodenotyetupdated){  // if too close to the next node but DG hasn't update to the next node OR already done the solution before but node is still not yet updated
        m_robotarrived_but_nodenotyetupdated = true;

        double dist_cur_to_nextnode = norm(dg_next_node_robot-dg_cur_node_robot);
        double p = min(2 / dist_cur_to_nextnode, 1.0); // what is (e.g.) 2 meter ratio with the distance between cur and next node  
        
        pub_pose.x = dg_cur_node_robot.x + (1.0+p) * (dg_next_node_robot.x - dg_cur_node_robot.x);
        pub_pose.y = dg_cur_node_robot.y + (1.0+p) * (dg_next_node_robot.y - dg_cur_node_robot.y);
...
bool DGRobot::makeSubgoal7(Pose2& pub_pose)  
{ 
    // in DeepGuider coordinate
    GuidanceManager::ExtendedPathElement cur_guide = m_guider.getCurExtendedPath();
    GuidanceManager::ExtendedPathElement next_guide = m_guider.getNextExtendedPath();
    ...
    ROS_INFO("[makeSubgoal7] DG Pose node_robot.x: %f, y:%f",dg_pose.x, dg_pose.y);
    
    Pose2 cur_node_dg = Point2(cur_guide);
    Pose2 next_node_dg = Point2(next_guide);

    ROS_INFO("[makeSubgoal7] cur_node_dg.x: %f, y:%f",cur_node_dg.x, cur_node_dg.y);
    ROS_INFO("[makeSubgoal7] next_node_dg.x: %f, y:%f",next_node_dg.x, next_node_dg.y);
    ...
    ROS_INFO("[makeSubgoal7] robot_pose node_robot.x: %f, y:%f",robot_pose.x, robot_pose.y);
    ...
    ROS_INFO("[makeSubgoal7] CurGuideIdx %d", m_guider.getCurGuideIdx());
    ...
    // m_prev_robot_pose;  //in robot coordinate
    // m_cur_robot_pose;  //in robot coordinate
    ROS_INFO("[makeSubgoal7] m_prev_robot_pose: <%f, %f>", m_prev_robot_pose.x, m_prev_robot_pose.y);
    ROS_INFO("[makeSubgoal7] m_cur_robot_pose: <%f, %f>", m_cur_robot_pose.x, m_cur_robot_pose.y);
    ...
    if (m_cur_head_node_id != next_guide.cur_node_id)  // new dg_next_node_robot
    {
        if(m_prev_node_id != 0) //NOT initial start (when m_prev_node_robot is not 0 anymore)
        {            
            ROS_INFO("[makeSubgoal7] prev_id: %zd, cur_id: %zd, next_id: %zd", m_prev_node_id, m_cur_head_node_id, next_guide.cur_node_id);
            ...
            // consider prev dx node is same with prev m_cur_node_dx
            m_prev_node_dx = m_cur_node_dx;
            
            // update m_cur_node_dx. Consider current dx node is current robot position
            m_cur_node_dx = robot_pose;

            // prev-cur dx node angle w.r.t. robot coord
            Pose2 prev_node_dx_dummy = m_prev_node_dx;
            prev_node_dx_dummy.x = m_prev_node_dx.x - 10;
            int deg_robot = m_guider.getDegree(prev_node_dx_dummy, m_prev_node_dx, m_cur_node_dx);

            // the next dx node is based on dg next node and the error
            m_next_node_dx.x = m_cur_node_dx.x + dist_dgcur_to_dgnext * cos(cx::cvtDeg2Rad(deg_robot+diff_deg_robot));
            m_next_node_dx.y = m_cur_node_dx.y + dist_dgcur_to_dgnext * sin(cx::cvtDeg2Rad(deg_robot+diff_deg_robot));
    ...
    // // new_theta for robot pose (facing the next node). Note: probably not used as for subgoal, we only need coordinate.
    // new_theta = robot_pose.theta + cx::cvtDeg2Rad(diff_deg_robot);  // current pose + how much to turn based on diff_deg_robot
    ROS_INFO("[makeSubgoal7] dist_robot_to_nextnode: %f", dist_robot_to_nextnode);
...
bool DGRobot::makeSubgoal8(Pose2& pub_pose)  
{ 
    // in DeepGuider coordinate
    GuidanceManager::ExtendedPathElement cur_guide = m_guider.getCurExtendedPath();
    GuidanceManager::ExtendedPathElement next_guide = m_guider.getNextExtendedPath();
    ...
    ROS_INFO("[makeSubgoal8] DG Pose node_robot.x: %f, y:%f",dg_pose.x, dg_pose.y);
    
    Pose2 cur_node_dg = Point2(cur_guide);
    Pose2 next_node_dg = Point2(next_guide);

    ROS_INFO("[makeSubgoal8] cur_node_dg.x: %f, y:%f",cur_node_dg.x, cur_node_dg.y);
    ROS_INFO("[makeSubgoal8] next_node_dg.x: %f, y:%f",next_node_dg.x, next_node_dg.y);
    ...
    ROS_INFO("[makeSubgoal8] robot_pose node_robot.x: %f, y:%f",robot_pose.x, robot_pose.y);
    ...
    ROS_INFO("[makeSubgoal8] CurGuideIdx %d", m_guider.getCurGuideIdx());
    ...
    // m_prev_robot_pose;  //in robot coordinate
    // m_cur_robot_pose;  //in robot coordinate
    ROS_INFO("[makeSubgoal8] m_prev_robot_pose: <%f, %f>", m_prev_robot_pose.x, m_prev_robot_pose.y);
    ROS_INFO("[makeSubgoal8] m_cur_robot_pose: <%f, %f>", m_cur_robot_pose.x, m_cur_robot_pose.y);
    ...
    if (m_cur_head_node_id != next_guide.cur_node_id)  // new dg_next_node_robot
    {
        if(m_prev_node_id != 0) //NOT initial start (when m_prev_node_robot is not 0 anymore)
        {            
            ROS_INFO("[makeSubgoal8] prev_id: %zd, cur_id: %zd, next_id: %zd", m_prev_node_id, m_cur_head_node_id, next_guide.cur_node_id);
    ...
    // // new_theta for robot pose (facing the next node). Note: probably not used as for subgoal, we only need coordinate.
    // new_theta = robot_pose.theta + cx::cvtDeg2Rad(diff_deg_robot);  // current pose + how much to turn based on diff_deg_robot
    ROS_INFO("[makeSubgoal8] dist_robot_to_nextnode: %f", dist_robot_to_nextnode);
...
bool DGRobot::makeSubgoal9(Pose2& pub_pose)  // makeSubgoal6 with plan B-2-2
{ 
    // in DeepGuider coordinate
    GuidanceManager::ExtendedPathElement cur_guide = m_guider.getCurExtendedPath();
    GuidanceManager::ExtendedPathElement next_guide = m_guider.getNextExtendedPath();
    ...
    ROS_INFO("[makeSubgoal9] DG Pose node_robot.x: %f, y:%f",dg_pose.x, dg_pose.y);
    
    Pose2 cur_node_dg = Point2(cur_guide);
    Pose2 next_node_dg = Point2(next_guide);
    Pose2 next_next_node_dg = Point2(next_next_guide);

    ROS_INFO("[makeSubgoal9] cur_node_dg.x: %f, y:%f",cur_node_dg.x, cur_node_dg.y);
    ROS_INFO("[makeSubgoal9] next_node_dg.x: %f, y:%f",next_node_dg.x, next_node_dg.y);
    ROS_INFO("[makeSubgoal9] next_next_node_dg.x: %f, y:%f",next_next_node_dg.x, next_next_node_dg.y);
    ...
    ROS_INFO("[makeSubgoal9] robot_pose node_robot.x: %f, y:%f",robot_pose.x, robot_pose.y);
    ...
    ROS_INFO("[makeSubgoal9] CurGuideIdx %d", m_guider.getCurGuideIdx());
    ...
    // m_prev_robot_pose;  //in robot coordinate
    // m_cur_robot_pose;  //in robot coordinate
    ROS_INFO("[makeSubgoal9] m_prev_robot_pose: <%f, %f>", m_prev_robot_pose.x, m_prev_robot_pose.y);
    ROS_INFO("[makeSubgoal9] m_cur_robot_pose: <%f, %f>", m_cur_robot_pose.x, m_cur_robot_pose.y);
    ...
    if (m_cur_head_node_id != next_guide.cur_node_id)  // new dg_next_node_robot
    {
        if(m_prev_node_id != 0) //NOT initial start (when m_prev_node_robot is not 0 anymore)
        {            
            ROS_INFO("[makeSubgoal9] prev_id: %zd, cur_id: %zd, next_id: %zd", m_prev_node_id, m_cur_head_node_id, next_guide.cur_node_id);
    ...
    // // new_theta for robot pose (facing the next node). Note: probably not used as for subgoal, we only need coordinate.
    // new_theta = robot_pose.theta + cx::cvtDeg2Rad(diff_deg_robot);  // current pose + how much to turn based on diff_deg_robot
    ROS_INFO("[makeSubgoal9] dist_robot_to_nextnode: %f", dist_robot_to_nextnode);

    ////////////////////////////////////////////////////////////////////////////////////////////////////
    ////////////////////////////////////////////////////////////////////////////////////////////////////
    // // Plan A-2: comment below
    // Plan B-2-2: uncomment below 
    Pose2 temp_m_next_node_dx = m_next_node_dx;
    if (dist_robot_to_nextnode < 5 || m_robotarrived_but_nodenotyetupdated){  // if too close to the next node but DG hasn't update to the next node OR already done the solution before but node is still not yet updated
        ...
        ROS_INFO("[makeSubgoal9] robot arrived but node not yet updated. Use next next node");
...
bool DGRobot::makeSubgoal10(Pose2& pub_pose)  // makeSubgoal6 with plan B-2-3
{ 
    // in DeepGuider coordinate
    GuidanceManager::ExtendedPathElement cur_guide = m_guider.getCurExtendedPath();
    GuidanceManager::ExtendedPathElement next_guide = m_guider.getNextExtendedPath();
    ...
    ROS_INFO("[makeSubgoal10] DG Pose node_robot.x: %f, y:%f",dg_pose.x, dg_pose.y);
    
    Pose2 cur_node_dg = Point2(cur_guide);
    Pose2 next_node_dg = Point2(next_guide);
    Pose2 next_next_node_dg = Point2(next_next_guide);

    ROS_INFO("[makeSubgoal10] cur_node_dg.x: %f, y:%f",cur_node_dg.x, cur_node_dg.y);
    ROS_INFO("[makeSubgoal10] next_node_dg.x: %f, y:%f",next_node_dg.x, next_node_dg.y);
    ROS_INFO("[makeSubgoal10] next_next_node_dg.x: %f, y:%f",next_next_node_dg.x, next_next_node_dg.y);
    ...
    ROS_INFO("[makeSubgoal10] robot_pose node_robot.x: %f, y:%f",robot_pose.x, robot_pose.y);
    ...
    ROS_INFO("[makeSubgoal10] CurGuideIdx %d", m_guider.getCurGuideIdx());
    ...
    if (m_cur_head_node_id != next_guide.cur_node_id)  // new dg_next_node_robot
    {
        if(m_prev_node_id != 0) //NOT initial start (when m_prev_node_robot is not 0 anymore)
        {            
            ROS_INFO("[makeSubgoal10] prev_id: %zd, cur_id: %zd, next_id: %zd", m_prev_node_id, m_cur_head_node_id, next_guide.cur_node_id);
    ...
    // // new_theta for robot pose (facing the next node). Note: probably not used as for subgoal, we only need coordinate.
    // new_theta = robot_pose.theta + cx::cvtDeg2Rad(diff_deg_robot);  // current pose + how much to turn based on diff_deg_robot
    ROS_INFO("[makeSubgoal10] dist_robot_to_nextnode: %f", dist_robot_to_nextnode);
    ...
    ////////////////////////////////////////////////////////////////////////////////////////////////////
    // // Plan A-2: comment below
    // Plan B-2-3: uncomment below 
    Pose2 temp_m_next_node_dx = m_next_node_dx;
    if (dist_robot_to_nextnode < 5 || m_robotarrived_but_nodenotyetupdated){  // if too close to the next node but DG hasn't update to the next node OR already done the solution before but node is still not yet updated
        m_robotarrived_but_nodenotyetupdated = true;
        ROS_INFO("[makeSubgoal10] robot arrived but node not yet updated. Use next next node");
        
        // use the next next node AND align the next-next node with the errorvector
        Pose2 aligned_next_next_node_robot;
        aligned_next_next_node_robot.x = dg_next_next_node_robot.x + m_errorvec_dgnode_curpose.x;  // TODO: remove this
        aligned_next_next_node_robot.y = dg_next_next_node_robot.y + m_errorvec_dgnode_curpose.y;
        Pose2 aligned_next_node_robot;
        aligned_next_node_robot.x = dg_next_node_robot.x + m_errorvec_dgnode_curpose.x;
        aligned_next_node_robot.y = dg_next_node_robot.y + m_errorvec_dgnode_curpose.y;

        Point2 aligned_next_next_node_robot_px = cvtRobottoMapcoordinate(aligned_next_next_node_robot); //cyan diamond
        cv::drawMarker(colormap, aligned_next_next_node_robot_px, cv::Vec3b(255, 255, 0), 3, 40, 5);
        
        // go by sub_goal_distance-dist_robot_to_nextnode towards the next next node
        double q = min((sub_goal_distance-dist_robot_to_nextnode) / norm(aligned_next_next_node_robot-aligned_next_node_robot), 1.0);
        m_next_node_dx.x = aligned_next_node_robot.x + q * (aligned_next_next_node_robot.x - aligned_next_node_robot.x);
        m_next_node_dx.y = aligned_next_node_robot.y + q * (aligned_next_next_node_robot.y - aligned_next_node_robot.y);
        Point2 dx_next_next_node_robot_px = cvtRobottoMapcoordinate(m_next_node_dx); //navy diamond
        cv::drawMarker(colormap, dx_next_next_node_robot_px, cv::Vec3b(128, 0, 0), 3, 40, 5);
...
bool DGRobot::makeSubgoal11(Pose2& pub_pose)  // makeSubgoal9 with "directly go to next node if drivable" (good for offline map)
{ 
    // in DeepGuider coordinate
    GuidanceManager::ExtendedPathElement cur_guide = m_guider.getCurExtendedPath();
    GuidanceManager::ExtendedPathElement next_guide = m_guider.getNextExtendedPath();
    ...
    ROS_INFO("[makeSubgoal11] DG Pose node_robot.x: %f, y:%f",dg_pose.x, dg_pose.y);
    
    Pose2 cur_node_dg = Point2(cur_guide);
    Pose2 next_node_dg = Point2(next_guide);
    Pose2 next_next_node_dg = Point2(next_next_guide);

    ROS_INFO("[makeSubgoal11] cur_node_dg.x: %f, y:%f",cur_node_dg.x, cur_node_dg.y);
    ROS_INFO("[makeSubgoal11] next_node_dg.x: %f, y:%f",next_node_dg.x, next_node_dg.y);
    ROS_INFO("[makeSubgoal11] next_next_node_dg.x: %f, y:%f",next_next_node_dg.x, next_next_node_dg.y);
    ...
    ROS_INFO("[makeSubgoal11] robot_pose node_robot.x: %f, y:%f",robot_pose.x, robot_pose.y);
    ...
    ROS_INFO("[makeSubgoal11] CurGuideIdx %d", m_guider.getCurGuideIdx());
    ...
    // m_prev_robot_pose;  //in robot coordinate
    // m_cur_robot_pose;  //in robot coordinate
    ROS_INFO("[makeSubgoal11] m_prev_robot_pose: <%f, %f>", m_prev_robot_pose.x, m_prev_robot_pose.y);
    ROS_INFO("[makeSubgoal11] m_cur_robot_pose: <%f, %f>", m_cur_robot_pose.x, m_cur_robot_pose.y);
    ...
    if (m_cur_head_node_id != next_guide.cur_node_id)  // new dg_next_node_robot
    {
        if(m_prev_node_id != 0) //NOT initial start (when m_prev_node_robot is not 0 anymore)
        {            
            ROS_INFO("[makeSubgoal11] prev_id: %zd, cur_id: %zd, next_id: %zd", m_prev_node_id, m_cur_head_node_id, next_guide.cur_node_id);
    ...
    // // new_theta for robot pose (facing the next node). Note: probably not used as for subgoal, we only need coordinate.
    // new_theta = robot_pose.theta + cx::cvtDeg2Rad(diff_deg_robot);  // current pose + how much to turn based on diff_deg_robot
    ROS_INFO("[makeSubgoal11] dist_robot_to_nextnode: %f", dist_robot_to_nextnode);
    ...
    if (dist_robot_to_nextnode < 5 || m_robotarrived_but_nodenotyetupdated){  // if too close to the next node but DG hasn't update to the next node OR already done the solution before but node is still not yet updated
        ...
        ROS_INFO("[makeSubgoal11] robot arrived but node not yet updated. Use next next node");
    ...
    // directly go to next node if drivable
    if (isSubPathDrivablev3(robotmap_erode, m_next_node_dx, robot_pose)){  // if drivable
        pub_pose.x = m_next_node_dx.x;
        pub_pose.y = m_next_node_dx.y;
        ROS_INFO("[makeSubgoal11] Directly go to the next node");
        ROS_INFO("Found subggoal: <%f, %f>", pub_pose.x, pub_pose.y);  // OUTPUT.. care about pub_pose in robot's coordinate
        Pose2 pub_pose_px = cvtRobottoMapcoordinate(pub_pose);
        cv::circle(colormap, pub_pose_px, 20, cv::Vec3b(255, 0, 255), 5);  // small purple circle
        cv::circle(colormap, pub_pose_px, 5, cv::Vec3b(255, 0, 255), 2);  // with robot real size
