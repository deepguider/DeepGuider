...
int DeepGuiderROS::readRosParam(const cv::FileNode& fn)
{
    ...
    if (topic_name_index >= 0 && topic_name_index < topic_names_set.size()) m_topicset_tagname = topic_names_set[topic_name_index];

    // Read Topic Setting
    if (!m_topicset_tagname.empty())
    {
        cv::FileNode fn_topic = fn[m_topicset_tagname];
        if (!fn_topic.empty())
        {
