...
#define RECOGNIZER Intersection3CameraClassifier
...
void procfunc(const char* image_file)
{
    ...
    test_image_run(recognizer, false, image_file);
...
int main()
{
    ...
    const char* image_path[] = {
        "data_intersection3camera_cls/intersection_omni.jpg",
        "data_intersection3camera_cls/nonintersection_omni.jpg"
    };
    ...
    if(threaded_run)
    {
		...
		std::thread* test_thread = new std::thread(procfunc, image_path[image_sel]);
        test_thread->join();
    }
    else
    {
        //procfunc(recording, rec_fps, video_path[video_sel]);
        procfunc(image_path[image_sel]);
    }
